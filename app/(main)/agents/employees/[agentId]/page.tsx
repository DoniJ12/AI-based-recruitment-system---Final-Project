...
const Employees = ({ params }: { params: { agentId: string } }) => {
  const router = useRouter();
  const { data, isLoading, isError } =
    trpc.employee.getAgentEmployeesManager.useInfiniteQuery({
      limit: 5,
      activeAgent: params.agentId,
    });
  const employees = data?.pages[0].items;
  console.log("EMPLOYEEES", employees);
  console.log("active agent", params.agentId);
                ...
                {employees?.map((item, index) => (
                  ...
                  >
                    ...
                    <TableCell
                      ...
                      onClick={() => router.push(`/agents/employee/${item.id}`)}
                    >
